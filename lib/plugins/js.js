...
PluginExport.addStream('concat', function($destFile)
{
    ...
    if (Utils.isProd())
    {
        $stream.pipe( GulpUglify(ModulesConfig.uglify) );
    }
...
{
    var $config = Confirge.read(Config.jshintConfig);
    $config = Confirge.extend({}, ModulesConfig.jshint, $config);

    var $stream = GulpJsHint($config);
    $stream.pipe( GulpJsCs(ModulesConfig.jscs) )
