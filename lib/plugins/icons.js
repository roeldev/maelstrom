...
function fontCreateConfig($iconFontConfig)
{
    ...
    if ($import && Path.isAbsolute($import))
    {
        $import = Path.relative($dest, $import);
    }
...
function fontPrepareCodepoints($codepoints)
{
    ...
    {
        $codepoint = $codepoints[$i].unicode[0];
...
PluginExport.addStream('font', function($templateConfig)
{
    ...
    return GulpIconFont($config).on('glyphs', fontHandleCodepoints);
});
