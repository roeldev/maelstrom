...
function exportStreamer($plugin)
{
    var $export = function($stream)
    {
        ...
        return $plugin.stream($stream, true, $args);
    };

    $export.src = function($src)
    {
        return Utils.extendArgs($src, $plugin.src());
    };

    $export.dest = function($dest)
    {
        return Utils.extendArgs($dest, $plugin.dest());
    };
...
Plugin.prototype =
{
     ...
     */
    'stream': function($stream, $alwaysReturnStream, $args)
    {
