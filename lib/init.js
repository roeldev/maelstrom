...
module.exports = function(Maelstrom)
{
    ...
    Init.loadPackage = function($package)
    {
        var $plugin;
        try
        {
            ...
            if (Maelstrom.utils.isVerbose())
            {
                GulpUtil.log('- Load package \'' +
                    Chalk.cyan($package) + '\'');
            }
        }
        catch ($err)
        {
            var $packageRootDir = Path.dirname(__dirname);
            var $pluginFile     = Path.dirname($packageRootDir) + Path.sep +
                                  $package + Path.sep + 'lib' +
                                  Path.sep + 'index.js';

            if (Maelstrom.utils.isVerbose())
            {
                GulpUtil.log('- Load dev package \'' +
                    Chalk.cyan($package) + '\': ' +
                    Chalk.magenta( Tildify($pluginFile) ));
            }
        ...
        Maelstrom[$package] = this.loadPlugin($plugin);
    };
