...
var Maelstrom = function()
{
    if (arguments.length > 0)
    {
        ...
        // add tasks to gulp
        Init.defaultTasks(Maelstrom);
        // create a default watch task
        Init.defaultWatcher(Maelstrom);
    }
    return Maelstrom;
};
...
Maelstrom.init = function($gulp, $customConfig)
{
    ...
    Maelstrom.config = Init.createConfig(Maelstrom, $customConfig);

    // load plugins and add to main maelstrom instance
    Init.loadPlugins(Maelstrom);
};
...
Maelstrom.task = function($taskName, $includeSubTasks)
{
    ...
    if (!_.isUndefined($task))
    {
        ...
        $result = Maelstrom.gulp.task($taskName, $task);
    }
...
Maelstrom.watch = function($taskName, $extraFiles, $extraTasks)
{
    ...
    if (!_.isUndefined($task))
    {
        var $files = Maelstrom.utils.extendArgs($extraFiles, $task.plugin.src()),
            $tasks = Maelstrom.utils.extendArgs($extraTasks, $taskName);
