...
function extendArgs($args, $default)
{
    var $result;
    if (_.isArray($args))
    {
        $result = $args;
        $result.shift($default);
    }
    else
    {
        $result = [$default];
...
{
    console.log('[00:00:00] init maelstrom');

    //console.log('maelstrom.init()');
    if (!Init.isGulpInstance($gulp))
    {
        console.log('[00:00:00] Make sure to pass an instance of gulp to maelstrom.init()');
        return false;
    }
...
{
    //console.log('maelstrom.task()');

    var $task = this.tasks[$taskName];
    if (!_.isUndefined($task))
    {
        // add the task function to gulp
        this.gulp.task($taskName, $task.task.apply(null, arguments));
    }
...
Maelstrom.watch = function($taskName, $extraFiles, $extraTasks)
{
    ...
    if (!_.isUndefined($task))
    {
        var $files = extendArgs($extraFiles, $task.plugin.src()),
            $tasks = extendArgs($extraTasks, $taskName);
