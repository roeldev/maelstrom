...
Maelstrom.init = function($gulp, $addTasks, $customConfig)
{
    ...
    Init.loadPlugins( Maelstrom._PLUGIN_DIR );
...
Maelstrom.watch = function($taskName, $extraFiles, $extraTasks)
{
    ...
    if (!_.isUndefined($task))
    {
        ...
        $files = Utils.extendArgs($extraFiles, $files);
        if ($files.length > 0)
        {
            // add the task to be watched by gulp
            $result = Maelstrom.gulp.watch($files,
                Utils.extendArgs($extraTasks, $taskName));
        }
        else
        {
 ...
 * @type {string}
 */
Maelstrom._PLUGIN_DIR = Path.resolve(__dirname, 'plugins/');
